...
import {
  ROLE_PERMISSIONS,
  type Permission,
} from "@/pages/dashboard/permissions";
...
export function usePermissions() {
  ...
  const currentUser = useMemo<AppUser | null>(
    () => (user ? authUserToAppUser(user) : null),
    [user],
  );
  ...
  const {
    ...
    canAll: baseCanAll,
    canAny: baseCanAny,
  } = usePermission(permissionUser, ROLE_PERMISSIONS);
  ...
  const canUpdateWorkOrderNotes = useCallback(
    (order: WorkOrder) => {
      ...
      if (can("workorders.update")) return true;
      return (
        can("workorders.update_notes") && order.assignedTo === currentUser.id
      );
    },

...
export function AnalyticsView({
  ...
  const activeYear = availableYears.includes(year)
    ? year
    : (availableYears[0] ?? year);
  const yearRecords = repairRecords.filter((r) =>
    r.date.startsWith(String(activeYear)),
  );
  ...
  const monthlyData = MONTHS.map((month, i) => {
    ...
    const recs = yearRecords.filter((r) =>
      r.date.startsWith(`${activeYear}-${mo}`),
    );
    return {
      month,
      cost: recs.reduce((s, r) => s + r.totalCost, 0),
      repairs: recs.length,
    };
  });
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <span className="text-xs font-mono px-3 py-1.5 bg-primary text-white rounded-lg">
            FY {activeYear}
          </span>
        </div>
      </div>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="p-5">
          ...
          <p className="text-3xl font-bold text-primary mt-1">{totalRepairs}</p>
      ...
      <Card className="overflow-hidden">
        <div className="px-5 py-4 border-b border-border">
          ...
          <p className="text-xs text-muted-foreground font-mono mt-0.5">
            Total spend per month — {activeYear}
          </p>
        </div>
        <div className="p-5">
              ...
              <PieChart>
                ...
                >
                  {categoryData.map((entry) => (
                    <Cell
                      key={entry.name}
                      fill={CATEGORY_COLORS[entry.name as IssueCategory]}
                    />
                  ))}
                </Pie>
                <Tooltip
              ...
              {categoryData.map((c) => (
                <div key={c.name} className="flex items-center gap-2">
                  <div
                    ...
                    style={{
                      background: CATEGORY_COLORS[c.name as IssueCategory],
                    }}
                  />
      ...
      <Card className="overflow-hidden">
        <div className="px-5 py-4 border-b border-border">
          <h3 className="font-semibold text-foreground">
            Repair Log — {activeYear}
          </h3>
          <p className="text-xs text-muted-foreground font-mono mt-0.5">
            All completed repair records, sorted by cost
          </p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/40">
                <th className="text-left px-5 py-2.5 text-xs font-mono text-muted-foreground uppercase tracking-wider">
                  Record
                </th>
                <th className="text-left px-3 py-2.5 text-xs font-mono text-muted-foreground uppercase tracking-wider">
                  Machine
                </th>
                <th className="text-left px-3 py-2.5 text-xs font-mono text-muted-foreground uppercase tracking-wider hidden md:table-cell">
                  Category
                </th>
                <th className="text-left px-3 py-2.5 text-xs font-mono text-muted-foreground uppercase tracking-wider hidden lg:table-cell">
                  Issue
                </th>
                <th className="text-left px-3 py-2.5 text-xs font-mono text-muted-foreground uppercase tracking-wider">
                  Date
                </th>
                <th className="text-right px-5 py-2.5 text-xs font-mono text-muted-foreground uppercase tracking-wider">
                  Cost
                </th>
              </tr>
            </thead>
                ...
                <tr key={r.id} className="hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-3 py-3">
                    ...
                    <p className="text-xs text-muted-foreground truncate max-w-[100px]">
                      {machines.find((m) => m.id === r.machineId)?.site}
                    </p>
                  </td>
                  <td className="px-3 py-3 hidden md:table-cell">
                    <span
                      ...
                      style={{
                        background: CATEGORY_COLORS[r.issueCategory] + "18",
                        color: CATEGORY_COLORS[r.issueCategory],
                      }}
                    >
                      {r.issueCategory}
                    </span>
                  ...
                  <td className="px-3 py-3 hidden lg:table-cell max-w-[220px]">
                    <p className="text-xs text-foreground truncate">
                      {r.issueDescription.slice(0, 70)}…
                    </p>
                  </td>
                  <td className="px-3 py-3 text-xs text-muted-foreground font-mono whitespace-nowrap">
                    {formatDate(r.date)}
                  </td>
                  <td className="px-5 py-3 text-right font-mono text-sm font-semibold text-foreground">
            ...
            <tfoot>
              <tr className="border-t-2 border-border bg-muted/30">
                <td
                  colSpan={5}
                  className="px-5 py-3 text-xs font-mono font-semibold text-foreground uppercase tracking-wider"
                >
                  Total
                </td>

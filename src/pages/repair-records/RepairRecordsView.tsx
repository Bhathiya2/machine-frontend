...
import { Card, FormField, inputCls, selectCls } from '@/pages/dashboard/components/DashboardUI'
import { formatDate, fmtCurrency } from '@/pages/dashboard/utils/formatters'
import type { RepairFormData } from './repairMapper'
import type { AppUser, IssueCategory, Machine, RepairPhoto, RepairRecord, WorkOrder } from '@/pages/dashboard/types'
...
export function RepairRecordsView({
  ...
  const submitCreate = async () => {
    if (!form.workOrderId || !form.machineId || !form.technicianId || !form.issueDescription.trim() || saving) return
    setSaving(true)
    try {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <Card className="p-4">
          <p className="text-xs font-mono text-muted-foreground uppercase tracking-widest">Records</p>
          <p className="text-2xl font-bold text-primary mt-0.5">{filtered.length}</p>
        </Card>
        <Card className="p-4">
          <p className="text-xs font-mono text-muted-foreground uppercase tracking-widest">Total Cost</p>
          <p className="text-2xl font-bold text-foreground mt-0.5">{fmtCurrency(totalCost)}</p>
        </Card>
        <Card className="p-4">
          <p className="text-xs font-mono text-muted-foreground uppercase tracking-widest">Parts Cost</p>
          <p className="text-2xl font-bold text-foreground mt-0.5">{fmtCurrency(totalParts)}</p>
        </Card>
        <Card className="p-4">
          <p className="text-xs font-mono text-muted-foreground uppercase tracking-widest">Labor Cost</p>
          <p className="text-2xl font-bold text-foreground mt-0.5">{fmtCurrency(totalCost - totalParts)}</p>
        </Card>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
        <div className="lg:col-span-2 space-y-2">
          {pageItems.length === 0 && <p className="text-sm text-muted-foreground text-center py-8">No repair records found</p>}
          {pageItems.map((r) => {
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className={`text-xs font-mono px-1.5 py-0.5 rounded ${isSelected ? 'bg-white/20 text-white' : 'bg-muted text-muted-foreground'}`}>
                    {r.issueCategory}
                  </span>
                </div>
                ...
                <div className={`flex items-center justify-between mt-2 text-xs ${isSelected ? 'text-white/70' : 'text-muted-foreground'}`}>
                  ...
                  <span className={`font-mono font-semibold ${isSelected ? 'text-white' : 'text-foreground'}`}>{fmtCurrency(r.totalCost)}</span>
                </div>
              </button>
            )
          })}
        ...
        <div className="lg:col-span-3">
          {!selectedRecord ? (
            <Card className="flex flex-col items-center justify-center h-64 text-center">
              ...
              <p className="text-sm text-muted-foreground">Select a repair record to view details and photos</p>
            </Card>
          ) : (
            <div className="space-y-4">
              <Card className="p-5">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span
                        ...
                        style={{
                          background: CATEGORY_COLORS[selectedRecord.issueCategory] + '20',
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <p className="text-2xl font-bold text-foreground">{fmtCurrency(selectedRecord.totalCost)}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-4 mt-4 pt-4 border-t border-border">
                  <div>
                    <p className="text-xs font-mono text-muted-foreground uppercase tracking-wider">Date</p>
                    <p className="text-sm font-medium text-foreground mt-0.5">{formatDate(selectedRecord.date)}</p>
                  </div>
                  <div>
                    <p className="text-xs font-mono text-muted-foreground uppercase tracking-wider">Technician</p>
                    <p className="text-sm font-medium text-foreground mt-0.5">{getUserName(selectedRecord.technicianId)}</p>
                  </div>
                  <div>
                    <p className="text-xs font-mono text-muted-foreground uppercase tracking-wider">Work Order</p>
                    <p className="text-sm font-medium text-foreground mt-0.5 font-mono">{selectedRecord.workOrderId}</p>
                  </div>
                </div>
              </Card>
              <Card className="overflow-hidden">
                <div className="px-5 py-3 border-b border-border flex items-center gap-2">
                  ...
                  <h3 className="font-semibold text-sm text-foreground">Parts Replaced</h3>
                </div>
                      ...
                      {selectedRecord.partsReplaced.map((p, i) => (
                        <div key={i} className="px-5 py-3 flex items-center justify-between gap-3">
                          <div>
                            ...
                            <p className="text-xs font-mono text-muted-foreground">{p.partNumber}</p>
                          </div>
                    ...
                    <div className="px-5 py-3 border-t border-border flex justify-between text-sm bg-muted/30">
                      <div>
                        ...
                        <span className="font-mono font-semibold">
                          {fmtCurrency(selectedRecord.partsReplaced.reduce((s, p) => s + p.cost, 0))}
                        </span>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Labor: </span>
                        <span className="font-mono font-semibold">{fmtCurrency(selectedRecord.laborCost)}</span>
                      </div>
                    </div>
                  </div>
                )}
              ...
              <Card className="overflow-hidden">
                <div className="px-5 py-3 border-b border-border flex items-center gap-2">
                  ...
                  <h3 className="font-semibold text-sm text-foreground">Photo Gallery</h3>
                  <span className="text-xs text-muted-foreground font-mono ml-auto">{selectedRecord.photos.length} photos</span>
                </div>
                {selectedRecord.photos.length === 0 ? (
                  <p className="px-5 py-4 text-sm text-muted-foreground italic">No photos attached</p>
                ) : (
                  <div className="p-4">
                          ...
                          {selectedRecord.photos
                            ...
                            .map((photo) => {
                              ...
                              return (
                                <button
                                  key={photo.id}
                                  onClick={() => setLightbox({ photos: selectedRecord.photos, startIndex: globalIdx })}
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-card w-full max-w-lg rounded-xl shadow-2xl border border-border max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-6 py-4 border-b border-border sticky top-0 bg-card">
              <div>
                <h2 className="font-semibold text-foreground">New Repair Record</h2>
                <p className="text-xs text-muted-foreground font-mono mt-0.5">Link to a work order</p>
              </div>
            ...
            <div className="px-6 py-5 space-y-4">
              <FormField label="Work Order">
                <select className={selectCls} value={form.workOrderId} onChange={(e) => onSelectWorkOrder(e.target.value)}>
              ...
              <FormField label="Machine">
                <input className={inputCls + ' font-mono bg-muted/30'} value={form.machineId} readOnly placeholder="Auto from work order" />
              </FormField>
              <div className="grid grid-cols-2 gap-3">
                ...
                <FormField label="Category">
                  <select
                    ...
                    onChange={(e) => setForm({ ...form, issueCategory: e.target.value as IssueCategory })}
                  >
              ...
              <div className="space-y-2">
                <p className="text-xs font-mono text-muted-foreground uppercase tracking-wider">Parts (optional)</p>
                {form.partsReplaced.length > 0 && (
                  <div className="space-y-1">
                    {form.partsReplaced.map((p, i) => (
                      <div key={i} className="flex items-center justify-between text-sm px-3 py-2 rounded border border-border bg-muted/20">
                        <span>
                          {p.name} {p.partNumber ? `(${p.partNumber})` : ''}
                        </span>
                        <div className="flex items-center gap-2">
                          <span className="font-mono">{fmtCurrency(p.cost)}</span>
                          <button
                            type="button"
                            onClick={() =>
                              setForm({
                                ...form,
                                partsReplaced: form.partsReplaced.filter((_, idx) => idx !== i),
                              })
                            }
                ...
                <div className="grid grid-cols-3 gap-2">
                  <input className={inputCls + ' text-xs'} placeholder="Part name" value={partName} onChange={(e) => setPartName(e.target.value)} />
                  <input className={inputCls + ' text-xs font-mono'} placeholder="Part #" value={partNumber} onChange={(e) => setPartNumber(e.target.value)} />
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-border sticky bottom-0 bg-card">
              <button onClick={() => setShowCreate(false)} className="px-4 py-2 text-sm text-muted-foreground hover:text-foreground">
                Cancel
              </button>
              <button
                onClick={submitCreate}
                disabled={!form.workOrderId || !form.machineId || !form.technicianId || !form.issueDescription.trim() || saving}

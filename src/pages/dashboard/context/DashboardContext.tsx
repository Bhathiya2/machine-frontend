...
export function DashboardProvider({ children }: { children: ReactNode }) {
  ...
  const createWorkOrder = useCallback(
    async (form: WorkOrderFormData, createdBy: string) => {
  ...
  const updateWorkOrder = useCallback(
    async (dbId: number, form: WorkOrderFormData) => {
      setWorkOrdersSaving(true);
      try {
        const order = await workOrderRepository.update(dbId, form);
        setWorkOrders((prev) =>
          prev.map((item) => (item.dbId === dbId ? order : item)),
        );
  ...
  const updateWorkOrderStatus = useCallback(
    async (dbId: number, status: WorkOrderStatus) => {
      setWorkOrdersSaving(true);
      try {
  ...
  const updateWorkOrderNotes = useCallback(
    async (dbId: number, notes: string) => {
      try {
  ...
  const addTechnicianNotes = useCallback(
    async (dbId: number, notes: string) => {
      setWorkOrdersSaving(true);
      try {
  ...
  const updateWorkOrderCosts = useCallback(
    async (dbId: number, entries: CostEntry[]) => {
      try {
        const order = await workOrderRepository.updateCostEntries(
          dbId,
          entries,
        );
        setWorkOrders((prev) =>
  ...
  const loadCheckInSessions = useCallback(async (workOrderId: number) => {
    try {
      const sessions =
        await workOrderRepository.getCheckInSessions(workOrderId);
      return sessions;
    } catch (e: any) {
      const message =
        e?.response?.data?.message ?? "Failed to load check-in sessions";
      toast.error(message);
      return [];
    }
  }, []);
  ...
  const updateMachine = useCallback(
    async (dbId: number, form: MachineFormData) => {
      setMachinesSaving(true);
      try {
        const machine = await machineRepository.update(dbId, form);
        setMachines((prev) =>
          prev.map((item) => (item.dbId === dbId ? machine : item)),
        );
  ...
  const updateTechnician = useCallback(
    async (dbId: number, form: TechnicianFormData) => {
      setTechniciansSaving(true);
      try {
  ...
  const createFaultReport = useCallback(
    async (form: FaultFormData) => {
      try {
  ...
  const dismissFaultReport = useCallback(async (dbId: number) => {
    try {
      ...
      setFaultReports((prev) =>
        prev.map((item) => (item.dbId === dbId ? updated : item)),
      );

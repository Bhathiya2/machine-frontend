...
import {
  ...
  fmtCurrency,
  warrantyStatus,
} from "@/pages/dashboard/utils/formatters";
import {
  machineStatusIcon,
  severityColor,
  statusColor,
...
export function MachineRegistryView({
  ...
  const [activeFormTab, setActiveFormTab] = useState<
    "general" | "certificate" | "technical"
  >("general");
  const [form, setForm] = useState<MachineFormData>(() =>
    EMPTY_FORM(currentUser.name),
  );
  ...
  const saveMachine = async () => {
    if (!form.id.trim() || !form.name.trim() || !form.installDate || saving)
      return;
  ...
  const machineMeta = (machineId: string) => {
    const repairCount = repairRecords.filter(
  ...
  const viewOrders = activeView
    ? workOrders.filter((order) => order.machineId === activeView.id)
    : [];
  const viewRepairs = activeView
    ? repairRecords.filter((record) => record.machineId === activeView.id)
    : [];
  const viewFaults = activeView
    ? faultReports.filter((fault) => fault.machineId === activeView.id)
    : [];
            ...
            <TableBody>
              {pageItems.length === 0 ? (
                ...
                pageItems.map((machine) => {
                  ...
                  const { repairCount, openFaultCount } = machineMeta(
                    machine.id,
                  );
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Fill in the machine specifications and installation
                    certificate details.
                  </p>
                </div>
              </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <FormField label="Factory Group">
                      <input
                        ...
                        onChange={(event) =>
                          setForm({ ...form, factoryGroup: event.target.value })
                        }
                      />
                    </FormField>
                    <FormField label="Factory">
                      <input
                        ...
                        value={form.factory}
                        onChange={(event) =>
                          setForm({ ...form, factory: event.target.value })
                        }
                      />
                    </FormField>
                  </div>
                  ...
                  <FormField label="Installed By">
                    <input
                      ...
                      onChange={(event) =>
                        setForm({ ...form, installedBy: event.target.value })
                      }
                    />
                  </FormField>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <FormField label="Reference Number">
                        <input
                          ...
                          value={form.cert_reference || ""}
                          onChange={(e) =>
                            setForm({ ...form, cert_reference: e.target.value })
                          }
                        />
                      </FormField>
                      <FormField label="Calibration Info">
                        <input
                          className={inputCls}
                          placeholder="e.g. ISO 9001 Standard"
                          value={form.cert_calibration || ""}
                          onChange={(e) =>
                            setForm({
                              ...form,
                              cert_calibration: e.target.value,
                            })
                          }
                        />
                      </FormField>
                      <FormField label="Warranty Period">
                        <select
                          ...
                          onChange={(e) =>
                            setForm({ ...form, cert_warranty: e.target.value })
                          }
                        >
                      ...
                      <FormField label="Contract Reference">
                        <input
                          className={inputCls}
                          placeholder="e.g. CNT-88421"
                          value={form.cert_contract || ""}
                          onChange={(e) =>
                            setForm({ ...form, cert_contract: e.target.value })
                          }
                        />
                      </FormField>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      <FormField label="System">
                        <input
                          ...
                          value={form.client_system || ""}
                          onChange={(e) =>
                            setForm({ ...form, client_system: e.target.value })
                          }
                        />
                      </FormField>
                      <FormField label="Customer Code">
                        <input
                          ...
                          value={form.client_customer_code || ""}
                          onChange={(e) =>
                            setForm({
                              ...form,
                              client_customer_code: e.target.value,
                            })
                          }
                        />
                      </FormField>
                      <FormField label="Job Title">
                        <select
                          ...
                          onChange={(e) =>
                            setForm({
                              ...form,
                              client_job_title: e.target.value,
                            })
                          }
                        >
                          <option value="">Select job title</option>
                          {form.client_job_title &&
                            !JOB_TITLES.includes(form.client_job_title) && (
                              <option value={form.client_job_title}>
                                {form.client_job_title}
                              </option>
                            )}
                      ...
                      <FormField label="Expired Date">
                        <input
                          type="date"
                          className={inputCls}
                          value={form.client_expired_date || ""}
                          onChange={(e) =>
                            setForm({
                              ...form,
                              client_expired_date: e.target.value,
                            })
                          }
                        />
                      </FormField>
                      <FormField label="Date of Manufacture">
                        <input
                          type="date"
                          className={inputCls}
                          value={form.client_date_of_manufacture || ""}
                          onChange={(e) =>
                            setForm({
                              ...form,
                              client_date_of_manufacture: e.target.value,
                            })
                          }
                        />
                      </FormField>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      <FormField label="Frequency (Hz)">
                        <input
                          ...
                          value={form.tech_freq || ""}
                          onChange={(e) =>
                            setForm({ ...form, tech_freq: e.target.value })
                          }
                        />
                      </FormField>
                      <FormField label="Voltage (V)">
                        <input
                          className={inputCls}
                          placeholder="400 V"
                          value={form.tech_voltage || ""}
                          onChange={(e) =>
                            setForm({ ...form, tech_voltage: e.target.value })
                          }
                        />
                      </FormField>
                      <FormField label="Amperage (A)">
                        <input
                          className={inputCls}
                          placeholder="32 A"
                          value={form.tech_amp || ""}
                          onChange={(e) =>
                            setForm({ ...form, tech_amp: e.target.value })
                          }
                        />
                      </FormField>
                      <FormField label="Total MC Power">
                        <input
                          className={inputCls}
                          placeholder="15 kW"
                          value={form.tech_total_mc_power || ""}
                          onChange={(e) =>
                            setForm({
                              ...form,
                              tech_total_mc_power: e.target.value,
                            })
                          }
                        />
                      </FormField>
                      <FormField label="UPS System">
                        <input
                          className={inputCls}
                          placeholder="Online 10kVA"
                          value={form.tech_ups || ""}
                          onChange={(e) =>
                            setForm({ ...form, tech_ups: e.target.value })
                          }
                        />
                      </FormField>
                      <FormField label="Chiller Cooling">
                        <input
                          className={inputCls}
                          placeholder="Water Cooled"
                          value={form.tech_chiller_cooling_system || ""}
                          onChange={(e) =>
                            setForm({
                              ...form,
                              tech_chiller_cooling_system: e.target.value,
                            })
                          }
                        />
                      </FormField>
                      <FormField label="Chiller Absorbed Power">
                        <input
                          className={inputCls}
                          placeholder="5.5 kW"
                          value={form.tech_chiller_absorbed_power || ""}
                          onChange={(e) =>
                            setForm({
                              ...form,
                              tech_chiller_absorbed_power: e.target.value,
                            })
                          }
                        />
                      </FormField>
                      <FormField label="Smoke Extractor">
                        <select
                          ...
                          onChange={(e) =>
                            setForm({
                              ...form,
                              tech_smoke_extractor: e.target.value,
                            })
                          }
                        >
                          <option value="">
                            Select smoke extractor status
                          </option>
                          {form.tech_smoke_extractor &&
                            !SMOKE_EXTRACTOR_OPTIONS.includes(
                              form.tech_smoke_extractor,
                            ) && (
                              <option value={form.tech_smoke_extractor}>
                                {form.tech_smoke_extractor}
                              </option>
                            )}
                      ...
                      <FormField label="Room Temp (°C)">
                        <input
                          className={inputCls}
                          placeholder="22°C"
                          value={form.tech_room_temp || ""}
                          onChange={(e) =>
                            setForm({ ...form, tech_room_temp: e.target.value })
                          }
                        />
                      </FormField>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <FormField label="Signed By (Name)">
                        <input
                          ...
                          value={form.sign_signed_by || ""}
                          onChange={(e) =>
                            setForm({ ...form, sign_signed_by: e.target.value })
                          }
                        />
                      </FormField>
                      <FormField label="Technician Signature (Name/Initials)">
                        <input
                          ...
                          value={form.sign_technician_signature || ""}
                          onChange={(e) =>
                            setForm({
                              ...form,
                              sign_technician_signature: e.target.value,
                            })
                          }
                        />
                      </FormField>
                        ...
                        <div className="flex items-center gap-6">
                          <label className="flex items-center gap-2 text-sm font-medium cursor-pointer">
                            <input
                              ...
                              checked={form.sign_completed || false}
                              onChange={(e) =>
                                setForm({
                                  ...form,
                                  sign_completed: e.target.checked,
                                })
                              }
                            />
                          ...
                          <label className="flex items-center gap-2 text-sm font-medium cursor-pointer">
                            <input
                              type="checkbox"
                              className="size-4 rounded accent-primary"
                              checked={form.sign_incompleted || false}
                              onChange={(e) =>
                                setForm({
                                  ...form,
                                  sign_incompleted: e.target.checked,
                                })
                              }
                            />
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="rounded-lg border border-border p-3">
                  ...
                  <p className="mt-1 text-sm font-medium">
                    {formatDate(activeView.installDate)}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    By {activeView.installedBy}
                  </p>
                </div>
              </div>

              {viewFaults.filter((fault) => fault.status === "Open").length >
                0 && (
                  ...
                  <div className="divide-y divide-border">
                    {viewFaults
                      ...
                      .map((fault) => (
                        ...
                        >
                          <Badge className={severityColor(fault.severity)}>
                            {fault.severity}
                          </Badge>
                  ...
                  <div className="mt-2 grid grid-cols-3 gap-3">
                    ...
                    <div>
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => onNavigate("repairs", activeView.id)}
                      >
                        View records
                      </Button>
                    </div>
                ...
                <FormField label="Severity">
                  <select
                    ...
                    onChange={(event) =>
                      setNewFault({
                        ...newFault,
                        severity: event.target.value as FaultSeverity,
                      })
                    }
                  >
                    <option>Low</option>
          ...
          <div className="w-full max-w-md rounded-xl border border-border bg-card shadow-2xl">
            ...
            <div className="px-6 py-5">
              <p className="text-sm text-muted-foreground">
                ...
                ({deleteTarget.name}) from the registry? This cannot be undone.
              </p>
            </div>
            <div className="flex justify-end gap-3 border-t border-border px-6 py-4">

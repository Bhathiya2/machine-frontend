...
export function FinanceView({
  ...
  const [currency, setCurrency] = useState<FinanceCurrency>(() => {
    ...
    return stored && CURRENCY_OPTIONS.includes(stored as FinanceCurrency)
      ? (stored as FinanceCurrency)
      : "BDT";
  });
  ...
  const currentMonthEntriesCount = useMemo(() => {
    ...
    return localWorkOrders
      .flatMap((w) => w.costEntries)
      .filter((e) => e.date.startsWith(ym)).length;
  }, [localWorkOrders, selectedYear, selectedMonth]);

  const grandTotals = useMemo(() => {
    const ym = `${selectedYear}-${selectedMonth}`;
    const all = localWorkOrders
      .flatMap((w) => w.costEntries)
      .filter((e) => e.date.startsWith(ym));
    return {
      transportation: all
      ...
      labor: all
        ...
        .reduce((s, e) => s + e.amount, 0),
      sparePart: all
        .filter((e) => e.category === "Spare Part")
        .reduce((s, e) => s + e.amount, 0),
      others: all
  ...
  const filteredWOs = useMemo(() => {
    ...
    return localWorkOrders.filter((w) => {
      ...
      const matchesMonth =
        w.createdAt.startsWith(ym) ||
        w.updatedAt.startsWith(ym) ||
        w.costEntries.some((e) => e.date.startsWith(ym));
    ...
    <div className="space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs font-mono text-muted-foreground">
              Track equipment, transport, accommodation & other costs per work
              order
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-6 gap-3">
        {COST_CATEGORIES.map((cat) => {
          ...
          const propName =
            cat === "Spare Part" ? "sparePart" : cat.toLowerCase();
          const val = grandTotals[
            propName as keyof typeof grandTotals
          ] as number;
          const meta = COST_CATEGORY_META[cat];
          return (
            <Card key={cat} className="p-4">
              <div
                className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-mono font-semibold mb-2 ${meta.bg} ${meta.text}`}
              >
                {cat}
              </div>
              ...
              <div className="w-full h-1 rounded-full mt-2 bg-muted">
                <div
                  ...
                  style={{
                    width: `${grandTotals.grand ? Math.min((val / grandTotals.grand) * 100, 100) : 0}%`,
        ...
        <Card className="p-4 border-primary/20 bg-primary/5">
          <p className="text-[10px] font-mono text-primary uppercase tracking-widest font-bold mb-2">
            Total — {MONTHS[parseInt(selectedMonth, 10) - 1]} {selectedYear}
          </p>
          <p className="text-xl font-bold text-primary">
            {fmtCurrency(grandTotals.grand)}
          </p>
          <p className="text-xs text-muted-foreground mt-1">
            {currentMonthEntriesCount} entries
          </p>
        </Card>
      </div>
      ...
      <Card className="overflow-hidden">
        <div className="px-5 py-4 border-b border-border">
          ...
          <p className="text-xs text-muted-foreground font-mono mt-0.5">
            Transportation · Accommodation · Labor · Spare Part · Others —
            stacked by month
          </p>
        </div>
        <div className="p-5">
          <ResponsiveContainer width="100%" height={200}>
            ...
            >
              ...
              <YAxis
                tickFormatter={(v) =>
                  v === 0
                    ? `${currencyMeta.symbol}0`
                    : `${currencyMeta.symbol}${(v / 1000).toFixed(0)}k`
                }
                tick={{
                  fontSize: 11,
                  fontFamily: "JetBrains Mono, monospace",
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
        ...
        <div className="lg:col-span-2 space-y-3">
          <div className="relative">
            ...
            />
            <input
              className="w-full pl-9 pr-4 py-2.5 rounded-lg border border-border bg-card text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 font-mono placeholder:font-sans placeholder:text-muted-foreground"
            ...
            {pageItems.map((wo) => {
              ...
              return (
                ...
                >
                  ...
                  <p
                    className={`text-sm font-semibold truncate ${isSelected ? "text-white" : "text-foreground"}`}
                  >
                    {wo.title}
                  </p>
                  <div
                  ...
                  >
                    <span className="font-mono">
                      {wo.machineId} · {machine?.site ?? "—"}
                    </span>
                    <span
                      className={`font-mono font-bold ${isSelected ? "text-white" : total > 0 ? "text-foreground" : "text-muted-foreground"}`}
                    >
                      {total > 0 ? fmtCurrency(total) : "No costs"}
                    </span>
                  </div>
                  {wo.costEntries.filter((e) =>
                    e.date.startsWith(`${selectedYear}-${selectedMonth}`),
                  ).length > 0 && (
                    <div className="flex gap-1 mt-2 flex-wrap">
                      {COST_CATEGORIES.filter((c) => catTotal(wo, c) > 0).map(
                        (c) => (
                          <span
                            key={c}
                            className={`text-[10px] font-mono px-1.5 py-0.5 rounded ${isSelected ? "bg-white/15 text-white" : `${COST_CATEGORY_META[c].bg} ${COST_CATEGORY_META[c].text}`}`}
                          >
                            {c}
                          </span>
                        ),
        ...
        <div className="lg:col-span-3">
          {!selectedWO ? (
            <Card className="flex flex-col items-center justify-center h-64 text-center">
              <span className="text-4xl font-bold leading-none text-muted-foreground/40 mb-3">
                {currencyMeta.symbol}
              </span>
              <p className="text-sm text-muted-foreground">
                Select a work order to manage its costs
              </p>
            </Card>
          ) : (
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <Badge className={woStatusColor(selectedWO.status)}>
                        {woStatusIcon(selectedWO.status)}
                        {selectedWO.status}
                      </Badge>
                    </div>
                    <h3 className="font-semibold text-foreground">
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      ...
                      {getMachine(selectedWO.machineId)?.site} · Assigned to{" "}
                      {getUserName(selectedWO.assignedTo)}
                    </p>
                  </div>
                  <div className="text-right shrink-0">
                    ...
                    <p className="text-2xl font-bold text-foreground mt-0.5">
                      {fmtCurrency(woTotal(selectedWO))}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      {
                        selectedWO.costEntries.filter((e) =>
                          e.date.startsWith(`${selectedYear}-${selectedMonth}`),
                        ).length
                      }{" "}
                      entries this month
                  ...
                  {COST_CATEGORIES.map((cat) => {
                    ...
                    return (
                      <div key={cat} className="text-center">
                        <div
                          className={`inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-mono font-semibold ${meta.bg} ${meta.text}`}
                        >
                          {cat}
                        </div>
                        <p
                          className={`text-sm font-bold mt-1 ${val > 0 ? meta.text : "text-muted-foreground"}`}
                        >
                          {fmtCurrency(val)}
                        </p>
                      </div>
              ...
              <Card className="overflow-hidden">
                ...
                {selectedWO.costEntries.filter((e) =>
                  ...
                  <div className="flex flex-col items-center justify-center py-12 text-center">
                    <span className="text-3xl font-bold leading-none text-muted-foreground/40 mb-2">
                      {currencyMeta.symbol}
                    </span>
                    <p className="text-sm text-muted-foreground">
                      No cost entries for{" "}
                    ...
                    {canEdit && (
                      <button
                        onClick={openAdd}
                        className="mt-3 text-xs font-mono text-foreground hover:text-foreground transition-colors"
                      >
                        + Add cost entry
                      </button>
                    )}
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-border bg-muted/40">
                          <th className="text-left px-4 py-2.5 text-xs font-mono text-muted-foreground uppercase tracking-wider">
                            Category
                          </th>
                          <th className="text-left px-4 py-2.5 text-xs font-mono text-muted-foreground uppercase tracking-wider">
                            Details
                          </th>
                          <th className="text-left px-4 py-2.5 text-xs font-mono text-muted-foreground uppercase tracking-wider hidden sm:table-cell">
                            Date
                          </th>
                          <th className="text-right px-4 py-2.5 text-xs font-mono text-muted-foreground uppercase tracking-wider">
                            Amount
                          </th>
                            ...
                            return (
                              ...
                              >
                                <td className="px-4 py-3">
                                  <span
                                    className={`inline-flex items-center px-2 py-0.5 rounded text-[11px] font-mono font-semibold ${meta.bg} ${meta.text}`}
                                  >
                                    {entry.category}
                                  </span>
                                </td>
                                ...
                                <td className="px-4 py-3 text-xs font-mono text-muted-foreground whitespace-nowrap hidden sm:table-cell">
                      ...
                      <tfoot>
                        <tr className="border-t-2 border-border bg-muted/30">
                          <td
                            colSpan={3}
                            className="px-4 py-3 text-xs font-mono font-semibold text-foreground uppercase"
                          >
                            Total
                          </td>
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-border">
              <div>
                ...
                <p className="text-xs font-mono text-muted-foreground mt-0.5">
                  {selectedWO.id} · {selectedWO.machineId}
                </p>
              </div>
              <button
            ...
            <div className="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">
              {editingEntry ? (
                <>
                  ...
                  {["Transportation", "Spare Part", "Others"].includes(
                    editForm.category,
                  ) && (
                    <FormField
                      label={
                        editForm.category === "Transportation"
                          ? "Route / Destination"
                          : editForm.category === "Spare Part"
                            ? "Part Name & Number"
                            : "Description"
                      }
                    >
                      <input
                        ...
                        placeholder={
                          editForm.category === "Transportation"
                            ? "e.g. Plant A to Plant B"
                            : editForm.category === "Spare Part"
                              ? "e.g. Servo Motor A06B"
                              : "e.g. Advance payment"
                        }
                    ...
                    <FormField label={`Amount (${moneyLabel})`}>
                      <div className="relative">
                        ...
                        <input
                          ...
                          min="0"
                          step="0.01"
                          className={inputCls + " pl-8 font-mono bg-muted/30"}
                          readOnly
                          value={
                            !isNaN(parseFloat(editForm.quantity)) &&
                            !isNaN(parseFloat(editForm.unitPrice))
                              ? (
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b border-border bg-muted/50">
                            <th className="text-left px-3 py-2 text-[10px] font-mono text-muted-foreground uppercase tracking-wider min-w-[180px] w-[200px]">
                              Type
                            </th>
                            <th className="text-left px-3 py-2 text-[10px] font-mono text-muted-foreground uppercase tracking-wider">
                              Details
                            </th>
                            <th className="text-right px-3 py-2 text-[10px] font-mono text-muted-foreground uppercase tracking-wider w-20">
                              Qty
                            </th>
                            <th className="text-right px-3 py-2 text-[10px] font-mono text-muted-foreground uppercase tracking-wider w-24">
                              Unit Price
                            </th>
                            <th className="text-right px-3 py-2 text-[10px] font-mono text-muted-foreground uppercase tracking-wider w-24">
                              Total
                          ...
                          {posRows.map((row, i) => (
                            <tr key={row.id}>
                              <td className="px-2 py-2">
                                <select
                                  className={
                                    "w-full px-2 py-1.5 bg-card border border-border rounded-lg text-sm appearance-none focus:outline-none focus:ring-2 focus:ring-primary/20"
                                  }
                                  value={row.category}
                                  onChange={(e) =>
                                    setPosRows(
                              ...
                              <td className="px-2 py-2">
                                {[
                                  ...
                                  <input
                                    className={
                                      inputCls + " text-xs py-1.5 h-8 w-full"
                                    }
                                    placeholder={
                                      row.category === "Transportation"
                                        ? "Route..."
                                        : row.category === "Spare Part"
                                          ? "Part Name..."
                                          : "Description..."
                                    }
                                    value={row.details}
                              ...
                              <td className="px-2 py-2">
                                <input
                                  ...
                                  className={
                                    inputCls +
                                    " text-xs py-1.5 h-8 font-mono text-right w-full"
                                  }
                                  placeholder="1"
                                  value={row.quantity}
                                  onChange={(e) =>
                              ...
                              <td className="px-3 py-2 text-right font-mono font-semibold text-foreground text-xs">
                                ...
                                !isNaN(parseFloat(row.unitPrice))
                                  ? fmtCurrency(

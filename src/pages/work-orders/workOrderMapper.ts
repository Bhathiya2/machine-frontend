import type { CreateWorkOrderDto, UpdateWorkOrderDto, WorkOrderApi } from '@/interfaces/all/workOrder'
...
export function apiWorkOrderToUi(api: WorkOrderApi): WorkOrder {
  return {
    ...
    notes: api.notes ?? '',
    technician_notes: api.technician_notes?.map(note => ({
      ...note,
...
export function formToCreateDto(form: WorkOrderFormData, createdBy: string): CreateWorkOrderDto {
  return {
